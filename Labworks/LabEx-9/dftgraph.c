...
Graph * createAdjMatrixOfGraph(Graph *g){
  ...
  printf("Enter the number of Workers and Connections:");
  ...
  for(i = 0; i < g->nEdges; i++){
    printf("Reading the Workers of a Connection(WorkerID, Connection):\n");
    scanf("%d %d", &u, &v);
    g->adjMatrix[u][v] = 1;
    g->adjMatrix[v][u] = 1;
  }
...
int main(void){
  ...
  printf("\nDepth First Traversal of the Workers: ");
